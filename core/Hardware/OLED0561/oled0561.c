...
void OLED_DISPLAY_ON (void){//OLED屏初始值设置并开显示
  ...
  0x00,0x10,//开始地址（双字节）       
	...
	0xa0,//设置扫描方向，水平反向
	0xc0//设置扫描方向，[0xc0,0xc1]垂直反向
	}; //
	I2C_SAND_BUFFER(OLED0561_ADD,COM,buf,30);
}
...
//	u8 j,t,c=0;
//	for(t=0;t<2;t++){
//		I2C_SAND_BYTE(OLED0561_ADD,COM,x); //页地址（从0xB0到0xB7）
//		I2C_SAND_BYTE(OLED0561_ADD,COM,y/16+0x10); //起始列地址的高4位
//		I2C_SAND_BYTE(OLED0561_ADD,COM,y%16);	//起始列地址的低4位
...
//	for(t=0;t<2;t++){
//		I2C_SAND_BYTE(OLED0561_ADD,COM,x); //页地址（从0xB0到0xB7）
//		I2C_SAND_BYTE(OLED0561_ADD,COM,XY_UC/16+0x10); //起始列地址的高4位
//		I2C_SAND_BYTE(OLED0561_ADD,COM,XY_UC%16);	//起始列地址的低4位
//		for(j=0;j<16;j++){ //整页内容填充

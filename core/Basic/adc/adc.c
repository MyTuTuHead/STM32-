...
void ADC_GPIO_Init(void){ //GPIO初始化设置
    ...
    GPIO_InitStructure.GPIO_Pin = ADC_CH0 | ADC_CH1 | ADC_CH4 | ADC_CH6; //!!!选择端口                        
...
void ADC_Configuration(void){ //初始化设置
	...
	ADC_InitStructure.ADC_NbrOfChannel = 4;//!!!顺序进行规则转换的ADC通道的数目（根据ADC采集通道数量修改）,1-16
	ADC_Init(ADC1, &ADC_InitStructure); //根据ADC_InitStruct中指定的参数初始化外设ADCx的寄存器
	//设置指定ADC的规则组通道，设置它们的转化顺序和采样时间
	//ADC1,ADC通道x,规则采样顺序值为y,采样时间为7.5周		y越大采样顺序越低	 
	ADC_RegularChannelConfig(ADC1, ADC_Channel_0, 1, ADC_SampleTime_1Cycles5);//!!! ADC1选择信道x,采样顺序y,采样时间n个周期
	ADC_RegularChannelConfig(ADC1, ADC_Channel_1, 2, ADC_SampleTime_1Cycles5);//!!! ADC1选择信道x,采样顺序y,采样时间n个周期
	ADC_RegularChannelConfig(ADC1, ADC_Channel_4, 3, ADC_SampleTime_1Cycles5);//!!! ADC1选择信道x,采样顺序y,采样时间n个周期
	ADC_RegularChannelConfig(ADC1, ADC_Channel_6, 4, ADC_SampleTime_1Cycles5);//!!! ADC1选择信道x,采样顺序y,采样时间n个周期
